  ...
  const handleAssign = async (e) => {
    ...
    try {
      const { error } = await supabase
        .from('payroll_records') // Saving to the new payroll table
        .insert([{ 
          ...
          payout_date: new Date().toISOString().split('T')[0] // Saves today's date
        }]);

      if (error) throw error;
      setFormMessage({ type: 'success', text: `Saved! ${empDetails.first_name} earned ₱${currentTotal} for ${quantity} items.` });
      setQuantity(1); // Reset
    } catch (err) {
    ...
    <div className="container py-5">
      <div className="row justify-content-center">
        <div className="col-md-6">
          <button className="btn btn-light mb-4 shadow-sm" onClick={onBack}>← Back</button>
          <div className="card shadow-lg border-0 rounded-4 p-5">
            ...
            {formMessage && (
              <div className={`alert alert-${formMessage.type} alert-dismissible fade show mb-4`}>
                {formMessage.text}
            ...
            <form onSubmit={handleAssign}>
              <div className="mb-3">
                ...
                <select className="form-select border-0 bg-light" value={selectedEmployee} onChange={(e) => setSelectedEmployee(e.target.value)} required>
              ...
              <div className="mb-3">
                ...
                <select className="form-select border-0 bg-light" value={selectedItem} onChange={(e) => setSelectedItem(e.target.value)} required>
              ...
              <button type="submit" className="btn btn-info text-white btn-lg w-100 fw-bold shadow-sm" disabled={isSubmitting}>
